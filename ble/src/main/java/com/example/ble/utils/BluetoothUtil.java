...
public class BluetoothUtil {
    ...
    public static void printServices(BluetoothGatt gatt) {
        if (gatt != null) {
            for (BluetoothGattService service : gatt.getServices()) {
                KLog.i(TAG, "service: " + service.getUuid());

                for (BluetoothGattCharacteristic characteristic : service.getCharacteristics()) {
                    KLog.d(TAG, "    characteristic: " + characteristic.getUuid()
                            + "   ------  value: " + Arrays.toString(characteristic.getValue())
                            + "   ------  properties: " + characteristic.getProperties());

                    for (BluetoothGattDescriptor descriptor : characteristic.getDescriptors()) {
                        KLog.v(TAG, "    descriptor: " + descriptor.getUuid()
    ...
    public static boolean refreshDeviceCache(BluetoothGatt gatt) {
        try {
            ...
            KLog.e(TAG, "An exception occured while refreshing device", e);
        }
        return false;
    }
